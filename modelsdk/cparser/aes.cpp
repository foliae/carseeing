...
NOTE:   String length must be evenly divisible by 16byte (str_len % 16 == 0)
                You should pad the end of the string with zeros if this is not
the case. For AES192/256 the key size is proportionally larger.
...
unsigned int aes_cbc_encrypt(unsigned char *pout, const unsigned char *pdata,
                             unsigned int nlen, const unsigned char *pkey,
                             unsigned int klen, const unsigned char *piv) {
  if (!pout || !pdata || !pkey)
    return ERR_AES_OUTPUT_BUFF_EMPTY;

  uint8_t iv[] = {0x70, 0xf1, 0x52, 0xb3, 0x04, 0x05, 0x06, 0x07,
                  0x28, 0x09, 0x5a, 0x0b, 0xec, 0x0d, 0xce, 0x0f};
  const uint8_t *pivb;
  ...
  memcpy(pout, pdata, sizeof(uint8_t) * nlen);

  struct AES_ctx ctx;
  AES_init_ctx_iv(&ctx, pkey, pivb);
  AES_CBC_encrypt_buffer(&ctx, pout, nlen);

  return ERR_AES_NO_ERROR;
}

unsigned int aes_cbc_decrypt(unsigned char *pout, const unsigned char *pdata,
                             unsigned int nlen, const unsigned char *pkey,
                             unsigned int klen, const unsigned char *piv) {
  if (!pout || !pdata || !pkey)
    return ERR_AES_OUTPUT_BUFF_EMPTY;
  ...
  struct AES_ctx ctx;
  AES_init_ctx_iv(&ctx, pkey, pivb);
  AES_CBC_decrypt_buffer(&ctx, pout, nlen);
